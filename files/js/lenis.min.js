...
// Функция для проверки, является ли элементом (избегаем ошибок)
function isElement(target) {
   return target instanceof Element || target instanceof HTMLElement;
}

// Отключаем Lenis внутри `data-disable-lenis`
document.addEventListener('wheel', (event) => {
   if (isElement(event.target) && event.target.closest('[data-disable-lenis]')) {
      event.stopPropagation(); // Остановить событие для Lenis
      return; // Дать браузеру нативно обработать скролл
   }
}, { passive: false });
...
// Функция для запуска Lenis
function startLenis() {
   lenis.start();  // Включаем прокрутку Lenis
}

// Функция для остановки Lenis
function stopLenis() {
   lenis.stop(); // Останавливаем прокрутку Lenis
}

// Получаем элемент слайдера
const fsListSlider = document.getElementById('fs-list-slider');

// Устанавливаем обработчики для событий "mouseenter" и "mouseleave"
if (fsListSlider) {
   // Инициализируем слайдер
   initSwiper();

   // При наведении на слайдер останавливаем Lenis
   fsListSlider.addEventListener('mouseenter', function() {
      stopLenis();  // Останавливаем прокрутку Lenis при наведении на слайдер
   });

   // При уходе слайдера снова включаем Lenis
   fsListSlider.addEventListener('mouseleave', function() {
      startLenis();  // Включаем прокрутку Lenis при уходе с слайдера
   });
}

// Функция для запуска анимации Lenis
function raf(time) {
   lenis.raf(time);
   requestAnimationFrame(raf);
}
